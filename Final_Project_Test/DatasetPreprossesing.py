    ...
    def genData2(self, windowsSize=10, stepSize=1):
        ...
        while True:
            ...
            while m < self.num_inter[n] + 1 - readRows:
                ...
                yield (tf.convert_to_tensor(data_user_window.recipe_id.tolist()[:-1]),
                       tf.convert_to_tensor(data_user_window.ingredient_ids.tolist()[:-1]),
                       tf.convert_to_tensor(data_user_window.recipe_features.tolist()[:-1])), tf.convert_to_tensor(
                    data.recipe_id.tolist()[-1])
